...
.Parameter Destination
    Backup path. If not specified, server BackupDirectory path is used
...
#>
    ...
    begin {
        # There are several assemblies with Microsoft.SqlServer.Management.Smo.Server type, so we'll determine exactly type we need
    ...
    process {
        ...
        foreach ($DB in $Database) {
            ...
            if ($ServerInstance.Version -eq $null) {
                Write-Error "Cannot connect to server $Server" -Category ConnectionError
                $NumberProcessed++
                continue
            }
            $ServerInstance.ConnectionContext.StatementTimeout = 65535
            if ([String]::IsNullOrEmpty($Destination)) {
                $Destination = $ServerInstance.BackupDirectory
            ...
            if ($BackupTransactionLog) {
                ...
                $Job = Start-Job -InitializationScript { Import-Module SQLPS -DisableNameChecking } -ScriptBlock {
...
Export-ModuleMember -Variable 'DatabaseServer'
